...
cd $TOPLEVEL
mkdir -p $BUILDDIR
mv linux*/$EPM_PACKAGENAME*.$EXTENSION $BUILDDIR/$EPM_PACKAGENAME-$PLUGINVERSION-$SUFFIX_BUILD_NUMBER.$EXTENSION
if [ "$SUFFIX" != "" ] ; then
    mv $BUILDDIR/$EPM_PACKAGENAME-$PLUGINVERSION-$SUFFIX_BUILD_NUMBER.$EXTENSION $BUILDDIR/$EPM_PACKAGENAME-$PLUGINVERSION-$SUFFIX_BUILD_NUMBER.$SUFFIX.$EXTENSION
fi
echo ""
echo "$BUILDDIR:"
ls -l $BUILDDIR
